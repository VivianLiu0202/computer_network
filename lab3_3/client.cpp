	...
	while (1){
		...
		if (recvSize > 0){
			...
			if (recvPacket.Check()){
                ...
                if(recvPacket.ack_no == base){
                    isVisited[recvPacket.ack_no] = true;
                    std::lock_guard<std::mutex> lock(mtx);
                    //如果是窗口下界base，窗口应该前移到未确认的最小序号处
                    for(int i = base+1;i<=base+WINODWS_SIZE;i++){
                        if(isVisited[i] == false){
                            base = i;
                            cout<<"\n<<<窗口移动>>>窗口移到base = "<<i<<endl;
                            break;
                        }
                    }
                    timers[recvPacket.seq_no] = -1;
				...
				else if (recvPacket.ack_no > base && recvPacket.ack_no<base+WINODWS_SIZE){ //如果数据包在窗口范围内
                    isVisited[recvPacket.ack_no] = true;//将该在窗口内的数据包标记为已接受
                    //接收到窗口内的包之后将计时器置为-1，表示该包已经乱序收到，不需要再计时
                    timers[recvPacket.seq_no] = -1;
                    std::lock_guard<std::mutex> lock(mtx);
                    cout << "\n#----接收RECV----#Client已收到【ack = " << recvPacket.ack_no << "】的确认报文" << endl;
                    //打印窗口情况
...
void ClientSendRecvFile(string filename, SOCKET clientSocket, SOCKADDR_IN serverAddr)
{
	...
	while (1){
		if (nextseqnum < base + WINODWS_SIZE && nextseqnum < pktSum)
		{
            ...
            timers[ContentPacket.seq_no] = clock();
            std::lock_guard<std::mutex> lock(mtx);
			...
			for (int i = base; i < nextseqnum; i++){
                if(isVisited[i] == false && ReSend == i)  //超时，重传超时的这个数据包
                {
                    ...
                    int result = sendto(clientSocket, (char*)&ContentPacket, sizeof(ContentPacket), 0, (sockaddr*)&serverAddr, sizeof(SOCKADDR_IN));
                    {
                        std::lock_guard<std::mutex> lock(mtx);
                        totalTimeOut++;
                        cout << "\n【重传数据包】seq = " << ContentPacket.seq_no << "的报文段已超时，正在重传......" << endl; 
                        cout<<"i="<<i<<endl;
                    }
                    timers[ContentPacket.seq_no] = clock();
                    break;//找到第一个超时的包，应该就是我们想要重传的包？？？
